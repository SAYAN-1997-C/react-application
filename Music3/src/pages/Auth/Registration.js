...
const Registration = () => {
  ...
  setTimeout(() => { return(
    <h2 className={`bg-green-300 p-3 rounded-2xl absolute left-10 bottom-10 w-[14%] ${ successMessage.length > 0 ? "flex" : "hidden"}`}>{successMessage}</h2>
  ) }, 5000);
  return (
    <Formik
      initialValues={{
        ...
        <Form>
          <div className="grid  xl:grid-cols-4 w-full md:h-screen p-3 bg-slate-900 xs:overflow-hidden">
            ...
            <div className="flex flex-col xl:col-span-2 px-5 pt-4">
              ...
              <h2 className={`bg-red-500p-3 rounded-2xl  w-[14%] ${ successMessage.length > 0 ? "flex" : "hidden"}`}>{errorMessage}</h2>
